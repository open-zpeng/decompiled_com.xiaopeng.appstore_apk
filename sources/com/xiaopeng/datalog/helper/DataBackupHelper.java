...
public class DataBackupHelper {
    ...
    public void deleteExpiredFile(final String str, Context context) {
        ...
        for (String str2 : list) {
            if (isExpiredFile(str2)) {
                arrayList.add(str2);
                LogUtils.d(TAG, "file: " + str2 + " is expired, deleted " + new File(DATA_FILE_DIR + str + "/" + context.getPackageName() + "/" + str2).delete());
            }
        }
        if (arrayList.size() > 0) {
    ...
    private File createNewFile(String str, Context context) {
        return new File(DATA_FILE_DIR + str + "/" + context.getPackageName() + "/" + str + "_" + DateUtils.formatDate10(System.currentTimeMillis()) + ".log");
    }

...
public class ApiRouter {
    ...
    private static RemoteOperator wakeRemoteService(UriStruct uriStruct) throws RemoteException {
        ...
        if (TextUtils.isEmpty(uriStruct.processTag)) {
            builder.scheme(ClientConstants.BINDER.SCHEME).authority(uriStruct.applicationId + ClientConstants.BINDER.API_SUFFIX);
        } else {
            builder.scheme(ClientConstants.BINDER.SCHEME).authority(uriStruct.applicationId + "." + uriStruct.processTag + ClientConstants.BINDER.API_SUFFIX);
        }
